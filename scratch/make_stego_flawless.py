...
width, height = 580, 290
img = Image.new('RGB', (width, height), color=(16, 32, 52))

flag = "CTF{stego_lsb_flawless_victory_804326}"
...
out_path = 'example/stego_flawless_804326.png'
stego_img.save(out_path)
print(f"Saved stego image to: {out_path}")

# Verify decoding
raw_check = list(stego_img.get_flattened_data() if hasattr(stego_img, 'get_flattened_data') else stego_img.getdata())
extracted_bits = []
for p in raw_check:
    for c in range(3):
