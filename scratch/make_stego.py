...
# Flag string to encode in LSB
flag = "CTF{stego_lsb_secret_flag_387613}"
print(f"Embedding Flag: {flag}")

# Convert flag to binary bit stream
flag_bytes = flag.encode('utf-8')
binary_bits = "".join(f"{b:08b}" for b in flag_bytes)
print(f"Total bits: {len(binary_bits)}")
...
for pixel in pixels:
    ...
    if bit_idx < len(binary_bits):
        r = (r & ~1) | int(binary_bits[bit_idx])
        bit_idx += 1
    if bit_idx < len(binary_bits):
        g = (g & ~1) | int(binary_bits[bit_idx])
        bit_idx += 1
    if bit_idx < len(binary_bits):
        b = (b & ~1) | int(binary_bits[bit_idx])
        bit_idx += 1
...
out_path = 'example/lsb_stego_test_387613.png'
stego_img.save(out_path)
print(f"Saved stego image to: {out_path}")

# Verify decoding
px = list(stego_img.getdata())
extracted_bits = []
for p in px:
    for c in range(3):
